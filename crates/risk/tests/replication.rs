...
use openfiat_types::{NodeRole, PeerId, PublicKey, SecurityService, ServiceId, ServiceType, Timestamp};
...
fn make_service(seed: u8, services: Rc<Registry<MemoryStore>>) -> RiskService<MemoryStore> {
    ...
    let gossip = openfiat_gossip::GossipService::new(node, event_store, keypair, vec![NodeRole::RiskIntelligenceProvider], Subscription::All);
...
async fn drive_until(services: &mut [RiskService<MemoryStore>], mut condition: impl FnMut(&[RiskService<MemoryStore>]) -> bool) {
...
async fn two_of_three_providers_flagging_a_scam_wallet_aggregates_to_reject() {
    ...
    let providers: Vec<Keypair> = seeds.iter().map(|&seed| Keypair::from_seed([seed; 32])).collect();
    let services = seeded_registries(&providers);
    let mut all: Vec<RiskService<MemoryStore>> = seeds.iter().map(|&seed| make_service(seed, Rc::clone(&services))).collect();
    ...
    all[0].publish("r-a", wallet.clone(), ProviderCategory::FraudIntelligence, RiskOutcome::Flagged, Severity::High, Confidence::High, "Known scam wallet", vec![], None).unwrap();
    // Provider B independently reaches the same conclusion.
    all[1].publish("r-b", wallet.clone(), ProviderCategory::FraudIntelligence, RiskOutcome::Flagged, Severity::High, Confidence::High, "Known scam wallet", vec![], None).unwrap();
    // Provider C never publishes anything for this wallet — "No Record".

    drive_until(&mut all, |services| services.iter().all(|s| s.for_wallet(&wallet).len() == 2)).await;

    for service in &all {
        let result = service.screen(&wallet);
        assert_eq!(result.highest_severity, Some(Severity::High));
        assert_eq!(result.active_flags.len(), 2, "both providers' flags should be visible on every node, including provider C's");
    }
}
