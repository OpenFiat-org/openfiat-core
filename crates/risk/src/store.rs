...
impl<S: KvStore> RiskIndex<S> {
    ...
    pub fn for_wallet(&self, wallet: &PeerId) -> Vec<RiskRecord> {
        self.all().into_iter().filter(|record| &record.wallet == wallet).collect()
    }
    ...
    pub fn apply_publish(&self, signed: SignedRiskPublish) -> Result<RiskRecordId, RiskError> {
        ...
        if !self.services.all().into_iter().any(|service| service.provider == publish.provider && matches!(service.service_type, ServiceType::Security(SecurityService::RiskIntelligenceProvider))) {
    ...
    pub fn screen(&self, wallet: &PeerId, now: Timestamp) -> ScreeningResult {
        let mut records: Vec<RiskRecord> = self.for_wallet(wallet).into_iter().filter(|record| record.is_current(now)).collect();
        records.sort_by_key(|record| record.published_at.as_millis());

        let cleared_at = records.iter().rev().find(|record| record.outcome == RiskOutcome::Cleared).map(|record| record.published_at.as_millis());
        let active_flags: Vec<RiskRecord> = records
            .into_iter()
            .filter(|record| record.outcome == RiskOutcome::Flagged && cleared_at.is_none_or(|cleared| record.published_at.as_millis() > cleared))
            .collect();
        let highest_severity = active_flags.iter().map(|record| record.severity).max();
        ScreeningResult { wallet: wallet.clone(), highest_severity, active_flags }
    }
    ...
    pub fn apply_event(&self, event: &EventEnvelope) {
        ...
        if event.event_type.as_str() != protocol::EVENT_FLAGGED && event.event_type.as_str() != protocol::EVENT_CLEARED {
            return;
        }
...
mod tests {
    ...
    fn flag(provider: &Keypair, id: &str, wallet: &PeerId, severity: Severity, published_at: Timestamp) -> RiskPublish {
        RiskPublish {
    ...
    fn an_unregistered_publisher_is_rejected() {
        ...
        let result = registry.apply_publish(SignedRiskPublish::sign(flag(&stranger, "r1", &wallet, Severity::High, Timestamp::now()), &stranger));
    ...
    fn a_duplicate_record_id_is_rejected() {
        ...
        let registry = RiskIndex::new(MemoryStore::new(), services);
        let wallet = peer_id_from_public_key(&Keypair::generate().public_key()).unwrap();
        registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r1", &wallet, Severity::High, Timestamp::now()), &provider)).unwrap();
        let result = registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r1", &wallet, Severity::Low, Timestamp::now()), &provider));
    ...
    fn screening_aggregates_to_the_worst_severity_among_current_flags() {
        ...
        let registry = RiskIndex::new(MemoryStore::new(), services);
        let wallet = peer_id_from_public_key(&Keypair::generate().public_key()).unwrap();
        registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r1", &wallet, Severity::Medium, Timestamp::from_millis(1)), &provider)).unwrap();
        registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r2", &wallet, Severity::Critical, Timestamp::from_millis(2)), &provider)).unwrap();
    ...
    fn a_later_clearance_supersedes_earlier_flags() {
        ...
        let registry = RiskIndex::new(MemoryStore::new(), services);
        let wallet = peer_id_from_public_key(&Keypair::generate().public_key()).unwrap();
        registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r1", &wallet, Severity::Critical, Timestamp::from_millis(1)), &provider)).unwrap();
        ...
        let clear = RiskPublish {
            ...
            ..flag(&provider, "r2", &wallet, Severity::Informational, Timestamp::from_millis(2))
        };
        registry.apply_publish(SignedRiskPublish::sign(clear, &provider)).unwrap();
    ...
    fn a_flag_after_a_clearance_is_not_superseded() {
        ...
        let clear = RiskPublish { outcome: RiskOutcome::Cleared, timestamp: Timestamp::from_millis(1), ..flag(&provider, "r1", &wallet, Severity::Informational, Timestamp::from_millis(1)) };
        registry.apply_publish(SignedRiskPublish::sign(clear, &provider)).unwrap();
        registry.apply_publish(SignedRiskPublish::sign(flag(&provider, "r2", &wallet, Severity::High, Timestamp::from_millis(2)), &provider)).unwrap();
