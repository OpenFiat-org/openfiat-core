...
use openfiat_advertisements::{AdvertisementId, AdvertisementService, AdvertisementStatus, Direction, PricingModel};
...
fn make_service(seed: u8) -> AdvertisementService<MemoryStore> {
    ...
    let gossip = openfiat_gossip::GossipService::new(node, event_store, keypair, vec![NodeRole::MerchantGateway], Subscription::All);
...
async fn drive_until(services: &mut [AdvertisementService<MemoryStore>], mut condition: impl FnMut(&[AdvertisementService<MemoryStore>]) -> bool) {
...
async fn a_created_and_then_disabled_advertisement_replicates_to_the_whole_cluster() {
    ...
    let mut all: Vec<AdvertisementService<MemoryStore>> = std::iter::once(hub).chain(leaves).collect();
    ...
    all[1]
        .create(
            ...
            PricingModel::Fixed { price: Amount::new(129_000_000, 6) },
            vec!["Mobile Money".to_string()],
        )
        .unwrap();

    drive_until(&mut all, |services| services.iter().all(|s| s.get(&ad_id).is_some())).await;
    for service in &all {
    ...
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&ad_id).unwrap().status == AdvertisementStatus::Disabled)).await;
}
