...
fn seeded_settlements(settlement_id: &SettlementId, buyer: &Keypair, seller: &Keypair) -> Rc<SettlementRegistry<MemoryStore>> {
    ...
    registry.apply_initiate(SignedSettlementInitiate::sign(initiate, buyer)).unwrap();
    registry
}

fn make_service(seed: u8, settlements: Rc<SettlementRegistry<MemoryStore>>) -> DisputeService<MemoryStore> {
...
async fn drive_until(services: &mut [DisputeService<MemoryStore>], mut condition: impl FnMut(&[DisputeService<MemoryStore>]) -> bool) {
...
async fn a_full_commit_reveal_cycle_reaches_a_consistent_resolution_across_the_cluster() {
    ...
    let settlement_registries: Vec<_> = seeds.iter().map(|_| seeded_settlements(&settlement_id, &buyer_kp, &seller_kp)).collect();
    let mut all: Vec<DisputeService<MemoryStore>> =
        seeds.iter().zip(settlement_registries.iter()).map(|(&seed, registry)| make_service(seed, Rc::clone(registry))).collect();
    ...
    let dispute_id = all[0].open("dispute-1", settlement_id, "payment not received").unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&dispute_id).is_some())).await;

    for arbitrator in &mut all[1..] {
        arbitrator.join_as_arbitrator(dispute_id.clone()).unwrap();
    }
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&dispute_id).unwrap().status == DisputeStatus::CaseLocked)).await;
    ...
    for ((arbitrator, vote), secret) in all[1..].iter_mut().zip(votes).zip(secrets) {
        arbitrator.commit_vote(dispute_id.clone(), vote, secret).unwrap();
    }
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&dispute_id).unwrap().status == DisputeStatus::RevealPhase)).await;

    for ((arbitrator, vote), secret) in all[1..].iter_mut().zip(votes).zip(secrets) {
        arbitrator.reveal_vote(dispute_id.clone(), vote, secret).unwrap();
    }
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&dispute_id).unwrap().status == DisputeStatus::Resolved)).await;
