...
use crate::events::{
    ArbitratorJoin, DisputeOpen, MutualSettlementAgree, SignedArbitratorJoin, SignedDisputeOpen, SignedMutualSettlementAgree, SignedVoteCommit,
...
impl<S: KvStore + 'static> DisputeService<S> {
    ...
    pub fn new(mut gossip: GossipService<S>, store: S, settlements: Rc<SettlementRegistry<S>>) -> Self {
    ...
    pub fn open(&mut self, id: impl Into<String>, settlement_id: SettlementId, reason: impl Into<String>) -> Result<DisputeId, DisputeError> {
        ...
        let signed = SignedDisputeOpen { signature: self.gossip.sign(&bytes), open };
        self.originate(protocol::EVENT_OPENED, &signed)?;
        Ok(signed.open.id)
    }

    pub fn join_as_arbitrator(&mut self, dispute_id: DisputeId) -> Result<(), DisputeError> {
        let join = ArbitratorJoin { dispute_id, arbitrator: self.gossip.node.local_peer_id(), arbitrator_public_key: self.gossip.public_key(), timestamp: Timestamp::now() };
        let bytes = wire::to_bytes(&join).map_err(|_| DisputeError::MalformedDispute)?;
        let signed = SignedArbitratorJoin { signature: self.gossip.sign(&bytes), join };
    ...
    pub fn commit_vote(&mut self, dispute_id: DisputeId, vote: Vote, secret: [u8; 32]) -> Result<(), DisputeError> {
        let commit = VoteCommit { dispute_id, arbitrator: self.gossip.node.local_peer_id(), commitment: commitment::compute(vote, &secret), timestamp: Timestamp::now() };
        let bytes = wire::to_bytes(&commit).map_err(|_| DisputeError::MalformedDispute)?;
        let signed = SignedVoteCommit { signature: self.gossip.sign(&bytes), commit };
        self.originate(protocol::EVENT_VOTE_COMMITTED, &signed)
    }

    pub fn reveal_vote(&mut self, dispute_id: DisputeId, vote: Vote, secret: [u8; 32]) -> Result<(), DisputeError> {
        let reveal = VoteReveal { dispute_id, arbitrator: self.gossip.node.local_peer_id(), vote, secret, timestamp: Timestamp::now() };
        let bytes = wire::to_bytes(&reveal).map_err(|_| DisputeError::MalformedDispute)?;
        let signed = SignedVoteReveal { signature: self.gossip.sign(&bytes), reveal };
        self.originate(protocol::EVENT_VOTE_REVEALED, &signed)
    }

    pub fn agree_to_mutual_settlement(&mut self, dispute_id: DisputeId) -> Result<(), DisputeError> {
        let agree = MutualSettlementAgree { dispute_id, party: self.gossip.node.local_peer_id(), timestamp: Timestamp::now() };
        let bytes = wire::to_bytes(&agree).map_err(|_| DisputeError::MalformedDispute)?;
        let signed = SignedMutualSettlementAgree { signature: self.gossip.sign(&bytes), agree };
        self.originate(protocol::EVENT_MUTUAL_SETTLEMENT_AGREED, &signed)
    }

    fn originate(&mut self, event_type: &str, payload: &impl serde::Serialize) -> Result<(), DisputeError> {
        let bytes = wire::to_bytes(payload).map_err(|_| DisputeError::MalformedDispute)?;
        let event_type = EventType::new(event_type).expect("dispute event names are all valid PascalCase identifiers");
        self.gossip
