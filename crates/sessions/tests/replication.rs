...
async fn drive_until(services: &mut [SessionService<MemoryStore>], mut condition: impl FnMut(&[SessionService<MemoryStore>]) -> bool) {
...
async fn a_session_establishes_renews_and_migrates_across_the_cluster() {
    ...
    let mut all: Vec<SessionService<MemoryStore>> = seeds.iter().map(|&seed| make_service(seed)).collect();
    ...
    let session_id = all[0].establish("sess-1", "web", vec!["trade".to_string()], None).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&session_id).is_some())).await;
    for service in &all {
        assert!(service.get(&session_id).unwrap().is_current(openfiat_types::Timestamp::now()));
    }

    all[0].renew(session_id.clone(), 1, None).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&session_id).unwrap().version == 1)).await;

    let new_host = identities[2].0.clone();
    all[0].migrate(session_id.clone(), new_host.clone(), 2).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&session_id).unwrap().host_node == new_host)).await;
