...
fn make_service(seed: u8) -> DiscoveryService<MemoryStore> {
    ...
    DiscoveryService::new(node, cache, keypair.public_key(), "1.0.0", vec![1000, 1100], vec![], 10)
}

/// Drive every service concurrently until `converged` returns true.
async fn drive_until(services: &mut [DiscoveryService<MemoryStore>], mut converged: impl FnMut(&[DiscoveryService<MemoryStore>]) -> bool) {
...
async fn a_five_node_cluster_converges_to_a_consistent_peer_set() {
    ...
    let mut services: Vec<DiscoveryService<MemoryStore>> = (1..=NODE_COUNT as u8).map(make_service).collect();
    ...
    drive_until(&mut services, |services| services.iter().all(|s| !s.listen_addresses().is_empty())).await;

    let bootstrap_addr: openfiat_network::Multiaddr = services[0].listen_addresses()[0].parse().unwrap();
    ...
    let all_peer_ids: std::collections::HashSet<_> = services.iter().map(|service| service.node.local_peer_id()).collect();
    assert_eq!(all_peer_ids.len(), NODE_COUNT, "every node must have a distinct peer ID");

    for service in &services {
        let known: std::collections::HashSet<_> = service.cache.all().unwrap().into_iter().map(|record| record.peer_id).collect();
        let expected: std::collections::HashSet<_> =
            all_peer_ids.iter().filter(|&peer_id| peer_id != &service.node.local_peer_id()).cloned().collect();
