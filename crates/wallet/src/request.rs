...
impl Wallet {
    pub fn sign_request<T: serde::Serialize>(
        ...
        payload: T,
        nonce: u64,
    ) -> Result<SignedRequest<T>, WalletError> {
...
pub fn verify_request<T: serde::Serialize>(
    ...
    let expected = peer_id_from_public_key(&signed.envelope.wallet_public_key)
    ...
    let age_ms = now
        .since(signed.envelope.timestamp)
        .ok_or(WalletError::RequestExpired)?;
