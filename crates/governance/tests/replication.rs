...
async fn drive_until(services: &mut [GovernanceService<MemoryStore>], mut condition: impl FnMut(&[GovernanceService<MemoryStore>]) -> bool) {
...
async fn a_proposal_passes_and_converges_across_the_cluster() {
    ...
    let mut all: Vec<GovernanceService<MemoryStore>> = seeds.iter().map(|&seed| make_service(seed)).collect();
    ...
    let proposal_id = all[0].create_proposal("ofp-1", "Increase Reservation Timeout", "Raise the validation window from 30 to 45 minutes.", ProposalCategory::Protocol).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&proposal_id).is_some())).await;

    for voter in &mut all[1..] {
        voter.cast_vote(proposal_id.clone(), VoteChoice::Approve, 1).unwrap();
    }
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&proposal_id).unwrap().votes.len() == 3)).await;
