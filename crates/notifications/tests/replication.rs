...
use openfiat_notifications::{DeliveryStatus, NotificationCategory, NotificationId, NotificationService, NotificationTrigger};
...
fn seeded_registry(provider: &Keypair, service_id: &str) -> Rc<Registry<MemoryStore>> {
    ...
    registry.apply_registration(SignedRegistration::sign(registration, provider)).unwrap();
    registry
}
...
fn make_service(seed: u8, services: Rc<Registry<MemoryStore>>) -> NotificationService<MemoryStore> {
    ...
    let gossip = openfiat_gossip::GossipService::new(node, event_store, keypair, vec![], GossipSubscription::All);
...
async fn drive_until(services: &mut [NotificationService<MemoryStore>], mut condition: impl FnMut(&[NotificationService<MemoryStore>]) -> bool) {
...
async fn a_subscription_and_a_delivery_report_replicate_across_the_cluster() {
    ...
    let registries: Vec<_> = seeds.iter().map(|_| seeded_registry(&provider_keypair, "svc-1")).collect();
    let mut all: Vec<NotificationService<MemoryStore>> = seeds.iter().zip(registries.iter()).map(|(&seed, registry)| make_service(seed, Rc::clone(registry))).collect();
    ...
    all[0].update_subscription(vec![NotificationCategory::Trading]).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.subscription(&wallet_peer_id).is_some())).await;
    for service in &all {
        assert!(service.subscription(&wallet_peer_id).unwrap().wants(NotificationTrigger::TradeCompleted));
        assert!(!service.subscription(&wallet_peer_id).unwrap().wants(NotificationTrigger::SnapshotAvailable));
    }

    let notification_id = NotificationId::new("notif-1");
    all[1]
        .report_delivery(notification_id.clone(), ServiceId::new("svc-1"), wallet_peer_id.clone(), NotificationTrigger::TradeCompleted, DeliveryStatus::Delivered)
        .unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.receipt(&notification_id).is_some())).await;
    for service in &all {
