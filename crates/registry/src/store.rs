...
impl<S: KvStore> Registry<S> {
    ...
    fn put(&self, record: &ServiceRecord) {
        if let Ok(bytes) = wire::to_bytes(record) {
            let _ = self.store.put(COLUMN_FAMILY, record.service_id.as_str().as_bytes(), &bytes);
        }
    }
    ...
    pub fn find_by_type(&self, service_type: ServiceType) -> Vec<ServiceRecord> {
        self.all().into_iter().filter(|record| record.service_type == service_type).collect()
    }
    ...
    pub fn apply_registration(&self, signed: SignedRegistration) -> Result<ServiceId, RegistryError> {
        signed.verify()?;
    ...
    pub fn apply_health_update(&self, signed: SignedHealthUpdate) -> Result<(), RegistryError> {
        let mut record = self.get(&signed.update.service_id).ok_or(RegistryError::ServiceNotFound)?;
        if record.provider != signed.update.provider {
            return Err(RegistryError::UnauthorizedUpdate);
        }
        let bytes = wire::to_bytes(&signed.update).map_err(|_| RegistryError::MalformedRegistration)?;
        verify(&record.provider_public_key, &bytes, &signed.signature).map_err(|_| RegistryError::InvalidSignature)?;
    ...
    pub fn apply_withdrawal(&self, signed: SignedWithdrawal) -> Result<(), RegistryError> {
        let record = self.get(&signed.withdrawal.service_id).ok_or(RegistryError::ServiceNotFound)?;
        if record.provider != signed.withdrawal.provider {
            return Err(RegistryError::UnauthorizedUpdate);
        }
        let bytes = wire::to_bytes(&signed.withdrawal).map_err(|_| RegistryError::MalformedRegistration)?;
        verify(&record.provider_public_key, &bytes, &signed.signature).map_err(|_| RegistryError::InvalidSignature)?;

        let _ = self.store.delete(COLUMN_FAMILY, signed.withdrawal.service_id.as_str().as_bytes());
        Ok(())
    }
    ...
    pub fn expire_stale(&self, threshold: Duration) -> usize {
        let cutoff = Timestamp::now().as_millis().saturating_sub(threshold.as_millis() as u64);
        let stale: Vec<ServiceId> =
            self.all().into_iter().filter(|record| record.last_health_update.as_millis() < cutoff).map(|record| record.service_id).collect();
        let count = stale.len();
        for id in stale {
...
mod tests {
    ...
    fn an_update_from_a_different_provider_is_rejected() {
        ...
        registry.apply_registration(SignedRegistration::sign(registration(&owner, "svc-1"), &owner)).unwrap();
    ...
    fn health_update_from_the_owner_is_accepted() {
        let registry = Registry::new(MemoryStore::new());
        let owner = Keypair::generate();
        let id = registry.apply_registration(SignedRegistration::sign(registration(&owner, "svc-1"), &owner)).unwrap();

        let update = HealthUpdate { service_id: id.clone(), provider: peer_id_from_public_key(&owner.public_key()).unwrap(), state: HealthState::Degraded, timestamp: Timestamp::now() };
        registry.apply_health_update(SignedHealthUpdate::sign(update, &owner)).unwrap();
    ...
    fn health_update_from_an_impostor_is_rejected() {
        ...
        let attacker = Keypair::generate();
        let update = HealthUpdate { service_id: id, provider: peer_id_from_public_key(&owner.public_key()).unwrap(), state: HealthState::Offline, timestamp: Timestamp::now() };
    ...
    fn withdrawal_removes_the_service() {
        ...
        let withdrawal = Withdrawal { service_id: id.clone(), provider: peer_id_from_public_key(&owner.public_key()).unwrap(), timestamp: Timestamp::now() };
        registry.apply_withdrawal(SignedWithdrawal::sign(withdrawal, &owner)).unwrap();
    ...
    fn expire_stale_removes_services_past_the_threshold() {
        ...
        let id = registry.apply_registration(SignedRegistration::sign(reg, &owner)).unwrap();
