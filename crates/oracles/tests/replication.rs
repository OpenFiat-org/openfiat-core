...
async fn drive_until(services: &mut [OracleService<MemoryStore>], mut condition: impl FnMut(&[OracleService<MemoryStore>]) -> bool) {
...
async fn three_providers_rates_aggregate_to_the_correct_median_across_the_cluster() {
    ...
    let mut all: Vec<OracleService<MemoryStore>> = seeds.iter().map(|&seed| make_service(seed, Rc::clone(&services))).collect();
    ...
    for (i, (service, rate)) in all.iter_mut().zip(rates).enumerate() {
        let data = OracleData::ExchangeRate { base: "USD".to_string(), quote: "KES".to_string(), rate };
        service.publish(format!("usd-kes-{i}"), data, 1, Duration::from_secs(60)).unwrap();
    }
    drive_until(&mut all, |services| services.iter().all(|s| s.all().len() == 3)).await;
