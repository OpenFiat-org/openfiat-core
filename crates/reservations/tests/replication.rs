...
fn seeded_ad_registry(ad_id: &AdvertisementId, merchant: &Keypair) -> Rc<AdvertisementRegistry<MemoryStore>> {
    ...
    registry.apply_create(SignedAdvertisementCreate::sign(create, merchant)).unwrap();
    registry
}

fn make_service(seed: u8, ad_registry: Rc<AdvertisementRegistry<MemoryStore>>) -> ReservationService<MemoryStore> {
...
async fn drive_until(services: &mut [ReservationService<MemoryStore>], mut condition: impl FnMut(&[ReservationService<MemoryStore>]) -> bool) {
...
async fn a_reservation_request_replicates_and_locks_liquidity_on_every_node() {
    ...
    let reservation_id = all[0].request("res-1", ad_id.clone(), Amount::new(2_000_000, 6)).unwrap();

    drive_until(&mut all, |services| services.iter().all(|s| s.get(&reservation_id).is_some())).await;
    ...
    assert_eq!(ad_registry_a.get(&ad_id).unwrap().available_liquidity, Amount::new(8_000_000, 6));
    assert_eq!(ad_registry_b.get(&ad_id).unwrap().available_liquidity, Amount::new(8_000_000, 6));

    // Cancel from the same node and confirm it replicates too.
    all[0].cancel(reservation_id.clone()).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&reservation_id).unwrap().state == ReservationState::Cancelled)).await;

    assert_eq!(ad_registry_a.get(&ad_id).unwrap().available_liquidity, Amount::new(10_000_000, 6));
    assert_eq!(ad_registry_b.get(&ad_id).unwrap().available_liquidity, Amount::new(10_000_000, 6));
}
