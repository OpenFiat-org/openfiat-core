...
async fn drive_until(services: &mut [IdentityService<MemoryStore>], mut condition: impl FnMut(&[IdentityService<MemoryStore>]) -> bool) {
...
async fn a_published_claim_replicates_and_converges_through_verify_and_revoke() {
    ...
    let mut all: Vec<IdentityService<MemoryStore>> = seeds.iter().map(|&seed| make_service(seed)).collect();
    ...
    let claim_id = all[0].publish("claim-1", ClaimType::Email, "merchant@example.com", false, None, None).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&claim_id).is_some())).await;
    for service in &all {
        assert_eq!(service.get(&claim_id).unwrap().verification_status, VerificationStatus::Unverified);
    }

    all[0].verify(claim_id.clone()).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&claim_id).unwrap().verification_status == VerificationStatus::Verified)).await;

    all[0].revoke(claim_id.clone()).unwrap();
    drive_until(&mut all, |services| services.iter().all(|s| s.get(&claim_id).unwrap().revoked)).await;
